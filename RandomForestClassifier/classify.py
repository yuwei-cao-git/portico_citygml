...
'''
example:
python classify.py '.\Models\FEAT_WITH_RGB.txt' '.\Models\MODEL_WITH_RGB.pkl' '.\Dataset\mantova_X70.las.section.segmented.rotated.features.colors.txt' '.\Predictions\prediction_mantova_X70_section_segmented_rotated_features_colors'
python classify.py '.\Models\FEAT_no_rgb.txt' '.\Models\MODEL_NO_RGB.pkl' '.\Dataset\mantova_x120_porch_2_feats_nocolor.txt' '.\Predictions\prediction_mantova_x120_porch_2_feats_nocolor'
'''
...
def main():
    ...
    X, Y = read_data(args.point_cloud)                         # Load data to classify
    
    print ('Classifying the dataset ...')
    
    Y_pred = model.predict(X[:, :])               # Classify the data
    acc = accuracy_score(Y, Y_pred)                       # Compute metrics and update best model
    ...
    print ('Saving ...')
    write_classification(X[:,:6], Y_pred, args.output_name)       # Output classification
    end = time.time()
